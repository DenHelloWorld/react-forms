...
const ReactHookFormAdvanced = ({ onSubmit }: ReactHookFormAdvancedProps) => {
  ...
  } = useForm<AdvancedFormInput, unknown, AdvancedFormData>({
  ...
  const onValid = async (data: AdvancedFormData) => {
    const image = await convertFile(data.image as File);
  ...
  const isTouchedWithError = (field: keyof AdvancedFormData) =>
