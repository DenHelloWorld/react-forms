...
const UncontrolledFormAdvanced = ({
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof AdvancedFormData, string>>
  >({});
  ...
  const onHandleSubmit = async (e: SyntheticEvent<HTMLFormElement>) => {
    ...
    const result = advancedFormSchema.safeParse(data);

    if (!result.success) {
      const fieldErrors: Partial<Record<keyof AdvancedFormData, string>> = {};

      for (const issue of result.error.issues) {
        const field = issue.path[0];
        if (typeof field === 'string') {
          fieldErrors[field as keyof AdvancedFormData] ??= issue.message;
        }
      }
    ...
    const image = await convertFile(result.data.image as File);
